...
//determines the range of possible node indices. set to uint to match the main code
typedef unsigned int NODE_T; 
//determines the possible number of networks to align
typedef unsigned char EDGE_T; //change to unsigned short for >256 networks

static bool _verbose;

std::string serializedFilesFolder = "autogenerated/serialized/";
...
enum class GraphType { gw, el };

GraphType getGraphType(const std::string& str) {
    switch (fnv::hash(str.c_str())) {
        case (fnv::hash(".gw")): return GraphType::gw;
        case (fnv::hash(".el")): return GraphType::el;
        default: throw std::runtime_error("Invalid graphType: "+str);
    }
    return GraphType::gw;
}
...
std::string name(const std::string & s) {
    ...
    if (i != std::string::npos) return s.substr(i+1);
    return s;
}
...
class ShadowGraph {
    ...
    typedef std::pair<NODE_T, EDGE_T> NodeWeightPair;
    ...
    void skipGWNodes(std::ifstream & reader, NODE_T numNodes) {
        std::string line;
    ...
    void loadGW(std::string filename) {
        ...
        adjList = std::vector<std::vector<NodeWeightPair> >(numNodes, std::vector<NodeWeightPair>(0));
        ...
        for (int i = 0; i < numEdges; i++) {
            ...
            node1--;
            node2--;
            this->adjList[node1].push_back(NodeWeightPair(node2,weight));
            this->adjList[node2].push_back(NodeWeightPair(node1,weight));
        }
        reader.close();
    ...
    void loadEL(std::string filename) {
        ...
        while(getline(reader, line)) {
            ...
            if (node_map.find(node1) == node_map.end()) {
                ...
                adjList.push_back(std::vector<NodeWeightPair>(0));
            }
            ...
            adjList[node_map[node1]].push_back(NodeWeightPair(node_map[node2],weight));
            adjList[node_map[node2]].push_back(NodeWeightPair(node_map[node1],weight));
        }
        this->nodes=nodes - 1;
        reader.close();
        writeBin(*this, filename);
    ...
    void writeBin(ShadowGraph& G, std::string& fileName) {
        std::string::size_type pos = fileName.find('.');
        if (pos != std::string::npos) {
            fileName = fileName.substr(0, pos);
        }
        mkdir("autogenerated/", ACCESSPERMS);
    ...
    void readBin(ShadowGraph& G, std::string& fileName) {
...
public:
    ...
    std::unordered_map<NODE_T, std::string> rev_node_map;
    std::vector<std::vector<NodeWeightPair> > adjList;
    NODE_T nodes = 0;
    ...
    void load(const std::string& filename) {
        ...
        GraphType g = getGraphType(extension);
        if (g == GraphType::gw) this->loadGW(filename);
        else this->loadEL(filename);
    }
};
...
void loadAlignment(std::vector<NODE_T> & alignment, std::string & s) {
    std::ifstream reader;
    std::string line;
    reader.open(s);
    std::getline(reader, line);
...
void output_GW_Format( std::vector<std::unordered_map<int, int>> &adjList){
    ...
    for (unsigned int i = 0; i < adjList.size(); ++i) {
        for (auto it = adjList[i].begin(); it != adjList[i].end(); ++it) {
            ...
            if ( it->second > (1L << 8*sizeof(EDGE_T))-10 ) {
                throw std::runtime_error("edge value too close to EDGE_T capacity, is it uchar or ushort?");
                continue;
            }
...
int main(int argc, const char** argv) {
    ...
    std::unordered_map<std::string, NODE_T> shadowName2Index; // TODO
    std::vector<std::string> shadowNames;
    ...
    shadow_graph::ShadowGraph tempGraph;
    for (unsigned int gi = 0; gi < network_files.size(); ++gi) {
        ...
        // Load alignment
        std::vector<NODE_T> tempAligOneline(0); 
        ...
        for (unsigned int peg = 0; peg < n; ++peg) {
            ...
            for (unsigned int j = 0; j < tempGraph.adjList[peg].size(); ++j) {
                NODE_T end_peg = tempGraph.adjList[peg][j].first;
