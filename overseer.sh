...
[ -f $OUTDIR/dir0/$NAME-shadow0.el ] || (./createShadow -s29262 --nodes-have-types --shadowNames Jurisica/networks/SHADOW.all.txt "$@" $OUTDIR/dir-init/*-shadow.align > $OUTDIR/dir0/$NAME-shadow0.el) || die "$NAME-shadow0.el network creation failed"
# Add 10 dummy "holes" for mRNAs at the end of the shadow network, each with weight 0 edges.
tail $OUTDIR/dir0/$NAME-shadow0.el | (awk '{printf "%s\tmRNA-%c\t0\n", $1, 64+NR}' >> $OUTDIR/dir0/$NAME-shadow0.el)
...
do
    ...
    ./createShadow -s29262 --nodes-have-types --shadowNames Jurisica/networks/SHADOW.all.txt "$@" $OUTDIR/dir$i1/*-shadow.align > $OUTDIR/dir$i1/$NAME-shadow$i1.el || die "$OUTDIR/dir$i1/$NAME-shadow$i1.el network creation failed"
    tail $OUTDIR/dir$i1/$NAME-shadow$i1.el | (awk '{printf "%s\tmRNA-%c\t0\n", $1, 64+NR}' >> $OUTDIR/dir$i1/$NAME-shadow$i1.el)
