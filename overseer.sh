...
yes | head -10 | (awk '{printf "DUMMY\tmRNA-%c\t0\n", 64+NR}' >> $OUTDIR/dir0/$NAME-shadow0.el)
...
do
	...
	sed 's/\.out\.align/.stdout/' $OUTDIR/dir$i/jobs-done.txt | fgrep -v -f - $OUTDIR/dir$i/jobs.txt | timed-run -`parse "60*($T_ITER+10)"` parallel_delay networks/$NAME-shadow$i/autogenerated/$NAME-shadow${i}_NodeNameToIndexMap.bin || # wait until that directory is created before parallel'ing
