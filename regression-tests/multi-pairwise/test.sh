...
if nice -19 ./multi-pairwise.sh ./sana.multi "-$MEASURE 1" $ITERS $minutes '-parallel 4' $DIR networks/syeast[01]*/*.el; then
    :
else
    EXIT_CODE=1
fi
...
echo "DIR is $DIR"
if gawk 'BEGIN{code=0}{k=$1;expect=(0.45-k/10);sum+=$3;if($3<expect)code=1}END{if(sum>'$MINSUM')code=0; exit(code)}' $DIR/MNC.txt; then
    :
else
    EXIT_CODE=1
