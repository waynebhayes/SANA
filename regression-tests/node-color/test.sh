...
(
    ...
    echo "echo The following error is expected >&2; '$SANA_EXE' -g1 yeast -g2 human -t 2 -s3 1 -fcolor1 $REG_DIR/yeast.col -fcolor2 $REG_DIR/human.col >$REG_DIR/y-h.col.stdout 2>&1 && exit 1"
    #after running it:
    ...
    echo "'$SANA_EXE' -fg1 $REG_DIR/covid.el -fg2 $REG_DIR/covid.el -t 4 -s3 1 -fcolor1 $REG_DIR/covid.col -fcolor2 $REG_DIR/covid.col > $REG_DIR/covid.stdout 2>&1 || exit 1; if grep -w 'E\|M\|N\|NSP1\|NSP10\|NSP11\|NSP12\|NSP13\|NSP14\|NSP15\|NSP2\|NSP4\|NSP5\|NSP5_C145A\|NSP6\|NSP7\|NSP8\|NSP9\|ORF10\|ORF3A\|ORF3B\|ORF6\|ORF7A\|ORF8\|ORF9B\|PROTEIN14\|S' sana.align | grep EN; then exit 1; fi"
    ...
    echo "'$SANA_EXE' -fg1 $REG_DIR/covid.el -fg2 $REG_DIR/covid.el -t 0.3 -s3 1 -lock-same-names > $REG_DIR/lock.stdout 2>&1 && exit 1"
    ...
    echo "'$SANA_EXE' -fg1 $REG_DIR/colorTest1.el -fcolor1 $REG_DIR/colorTest1.col -fg2 $REG_DIR/colorTest2.el -fcolor2 $REG_DIR/colorTest2.col -s3 1 -t 2 > $REG_DIR/colorTest.stdout 2>&1 || exit 1"
    ...
    echo "'$SANA_EXE' -fg1 $REG_DIR/colorTest1.el -fg2 $REG_DIR/colorTest2.el -lock $REG_DIR/lockTest.lock -s3 1 -t 2 > $REG_DIR/colorTest2.stdout 2>&1 || exit 1"
