#!/bin/sh
echo "We expect two of the following to die with an error:" >&2
...
(
    ...
    echo " $SANA_EXE -g1 yeast -g2 human -t 2 -s3 1 -fcolor1 $REG_DIR/yeast.col -fcolor2 $REG_DIR/human.col -o $REG_DIR/y-h.col >$REG_DIR/y-h.col.stdout 2>&1 && exit 1"
    ...
    echo "$SANA_EXE -fg1 $REG_DIR/covid.el -fg2 $REG_DIR/covid.el -t 0.3 -s3 1 -lock-same-names -o $REG_DIR/lock > $REG_DIR/lock.stdout 2>&1 && exit 1"
    ...
    echo "$SANA_EXE -fg1 $REG_DIR/colorTest1.el -fcolor1 $REG_DIR/colorTest1.col -fg2 $REG_DIR/colorTest2.el -fcolor2 $REG_DIR/colorTest2.col -s3 1 -t 2 -o $REG_DIR/colorTest1 > $REG_DIR/colorTest1.stdout 2>&1 || exit 1"
    ...
    echo "$SANA_EXE -fg1 $REG_DIR/colorTest1.el -fg2 $REG_DIR/colorTest2.el -lock $REG_DIR/lockTest.lock -s3 1 -t 2 -o $REG_DIR/colorTest2 > $REG_DIR/colorTest2.stdout 2>&1 || exit 1"
