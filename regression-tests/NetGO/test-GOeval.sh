...
if GOeval.sh -ebm -pa MM HS $TMPDIR/MMusculus-3.4.164.el $TMPDIR/HSapiens-3.4.164.el $TMPDIR/go.obo $TMPDIR/gene2go regression-tests/NetGO/MMHS1ec_60.align | fgrep -v '***' | tee $TMPDIR/GOeval.out | diff -b - <(unxz < regression-tests/NetGO/GOeval.MMHS.correct.xz) > $TMPDIR/diff.out; then
    : # do nothing, success
else
    EXIT_CODE="$?"
