#!/bin/sh
DIR=regression-tests/shadow
#creates a shadow from random alignments for 4 versions of the yeast network
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $DIR/start/yeastStartShadow.txt > $DIR/start/sana.stdout
...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $DIR/next/yeastNextShadow.txt > $DIR/next/sana.stdout
...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $DIR/startWithCols/yeastStartShadowWithCols.txt > $DIR/startWithCols/sana.stdout
#what some statistics should show:
#adjLists size: 1114
#color group sizes: 1090 23 1
#totalEdgeWeight: 39116

./$SANA_EXE.multi -t 0 -mode shadow -fshadow $DIR/nextWithCols/yeastNextShadowWithCols.txt > $DIR/nextWithCols/sana.stdout
