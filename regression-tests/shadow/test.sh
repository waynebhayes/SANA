...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/start/yeastStartShadow.txt > $REG_DIR/start/sana.stdout
...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/next/yeastNextShadow.txt > $REG_DIR/next/sana.stdout
...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/startWithCols/yeastStartShadowWithCols.txt > $REG_DIR/startWithCols/sana.stdout
#what some statistics should show:
#adjLists size: 1114
#color group sizes: 1090 23 1
#totalEdgeWeight: 39116

./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/nextWithCols/yeastNextShadowWithCols.txt > $REG_DIR/nextWithCols/sana.stdout
