...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/yeastStartShadow.txt > $TMPDIR/start.stdout 2>&1
...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/yeastNextShadow.txt > $TMPDIR/next.stdout 2>&1
...
./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/yeastStartShadowWithCols.txt > $TMPDIR/start.cols.stdout 2>&1
#what some statistics should show:
#adjLists size: 1114
#color group sizes: 1090 23 1
#totalEdgeWeight: 39116

./$SANA_EXE.multi -t 0 -mode shadow -fshadow $REG_DIR/yeastNextShadowWithCols.txt > $TMPDIR/next.cols.stdout 2>&1
