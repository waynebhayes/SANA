...
#[ -f $OUTDIR/dir0/$NAME-shadow0.$CrShOutType ] || (./createShadow -s$MAX_NODES $TYPEcreateSh "$@" $OUTDIR/dir000/*-shadow.out   >$OUTDIR/dir0/$NAME-shadow0.$CrShOutType) || die "$NAME-shadow0.$CrShOutType network creation failed"
...
if true; then
	...
	ls "$@" | awk '{file=$0;gsub(".*/",""); gsub(".el$",""); gsub(".gw$",""); printf "mkdir -p '$OUTDIR/dir000';'"/usr/bin/time -v timed-run -$TIME_LIMIT $SANA $TYPEargs"' -multi-iteration-only -t 0 -s3 0 -ses 1 -fg1 %s -fg2 '$OUTDIR/dir0/$NAME-shadow0.$CrShOutType' -o '$OUTDIR'/dir000/%s >'$OUTDIR'/dir000/%s.stdout 2>'$OUTDIR'/dir000/%s.stderr\n", file,$0,$0,$0}' | fgrep -v -f $OUTDIR/dir000/schedule.done | tee $OUTDIR/dir000/jobs.txt | eval timed-run -$TIME_LIMIT2 $PARALLEL #parallel_delay networks/$NAME-shadow0/autogenerated/$NAME-shadow0_NodeNameToIndexMap.bin
...
do
    ...
    #./createShadow -s$MAX_NODES "$@" $OUTDIR/dir$i1/*-shadow.out >$OUTDIR/dir$i1/$NAME-shadow$i1.$CrShOutType || die "$OUTDIR/dir$i1/$NAME-shadow$i1.$CrShOutType network creation failed"
