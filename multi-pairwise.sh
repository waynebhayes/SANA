...
TYPEargs=''
TYPEcreateSh=''
SHEX=gw # becomes el if TYPES
while echo "X$1" | grep '^X-' >/dev/null; do
    case "$1" in
    -H) HILLCLIMB=$2; shift;; # in addition to the shift that'll happen below
    -nodes-have-types) SHEX=el;
	TYPES=true; TYPEargs='-nodes-have-types -lock-same-names';
	TYPEcreateSh='--nodes-have-types --shadowNames Jurisica/SHADOW.all.txt'
	;;
...
mkdir -p $OUTDIR/dir0
[ -f $OUTDIR/dir0/$NAME-shadow0.$SHEX ] || (./createShadow -s$MAX_NODES $TYPEcreateSh "$@" $OUTDIR/dir000/*.out >$OUTDIR/dir0/$NAME-shadow0.$SHEX) || die "$NAME-shadow0.$SHEX network creation failed"
#[ -f $OUTDIR/dir0/$NAME-shadow0.el ] || (./createShadow -s29315 --nodes-have-types --shadowNames Jurisica/SHADOW.all.txt "$@" $OUTDIR/dir000/*-shadow.align > $OUTDIR/dir0/$NAME-shadow0.$SHEX) || die "$NAME-shadow0.$SHEX network creation failed"
...
if true; then
	...
	ls "$@" | awk '{file=$0;gsub(".*/",""); gsub(".el$",""); gsub(".gw$",""); printf "mkdir -p '$OUTDIR/dir000';'"/bin/time -v timed-run -$TIME_LIMIT $SANA $TYPEargs"' -multi-iteration-only -t 0 -s3 0 -ses 1 -fg1 %s -fg2 '$OUTDIR/dir0/$NAME-shadow0.$SHEX' -o '$OUTDIR'/dir000/%s >'$OUTDIR'/dir000/%s.stdout 2>'$OUTDIR'/dir000/%s.stderr\n", file,$0,$0,$0}' | fgrep -v -f $OUTDIR/dir000/schedule.done | tee $OUTDIR/dir000/jobs.txt | eval timed-run -$TIME_LIMIT2 $PARALLEL #parallel_delay networks/$NAME-shadow0/autogenerated/$NAME-shadow0_NodeNameToIndexMap.bin
...
do
    ...
    i1=`expr $ITER + 1`
    if [ -f  $OUTDIR/dir$i1/$NAME-shadow$i1.$SHEX ]; then continue; fi
    ...
    ./createShadow -s$MAX_NODES $TYPEcreateSh "$@" $OUTDIR/dir$i1/*.out >$OUTDIR/dir$i1/$NAME-shadow$i1.$SHEX || die "$OUTDIR/dir$i1/$NAME-shadow$i1.$SHEX network creation failed"
