...
parallel() { tee /dev/tty | /home/wayne/bin/bin.x86_64/parallel "$@"
}
lss() {
...
case "$JOB" in
    -parallel*) PARALLEL='tee /dev/tty | parallel -s bash '`echo $JOB | awk '{print $NF}'`;;
...
if true; then
	...
	ls "$@" | awk '{file=$0;gsub(".*/",""); gsub(".el$",""); gsub(".gw$",""); printf "mkdir -p '$OUTDIR/dir000';'"timed-run -$TIME_LIMIT $SANA $TYPEargs"' -multi-iteration-only -t 0 -s3 0 -ses 1 -fg1 %s -fg2 '$OUTDIR/dir0/$NAME-shadow0.el' -o '$OUTDIR'/dir000/%s >'$OUTDIR'/dir000/%s.stdout\n", file,$0,$0}' | fgrep -v -f $OUTDIR/dir000/schedule.done | tee $OUTDIR/dir000/jobs.txt | eval timed-run -$TIME_LIMIT2 $PARALLEL #parallel_delay networks/$NAME-shadow0/autogenerated/$NAME-shadow0_NodeNameToIndexMap.bin
...
do
	...
	named-col-prog 'if(name!="'$bg'")next; ITER='$ITER';NUM_ITERS='$NUM_ITERS'; e0=log(tinitial);e1=log(tfinal);printf "mkdir -p '$OUTDIR/dir$i1';'"timed-run -$TIME_LIMIT $SANA $TYPEargs"' -multi-iteration-only -s3 0 -ses 1 -t '$T_ITER' -fg1 '$g' -fg2 '$OUTDIR/dir$ITER/$NAME-shadow$ITER.el' -tinitial %g -tdecay %g -o '$OUTDIR/dir$i1/$bg-shadow' >'$OUTDIR/dir$i1/$bg-shadow.stdout' -startalignment '$OUTDIR/dir$ITER/$bg-shadow.align'\n", tinitial*exp((e1-e0)*ITER/NUM_ITERS),tdecay/NUM_ITERS' $OUTDIR/dir000/schedule.tsv
