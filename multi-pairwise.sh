...
SHADOW_GRAPH="$OUTDIR/dir0/$NAME-shadow0.gw"
ShadowConfig $SHADOW_GRAPH $# "$@" 1 $OUTDIR/dir-init/*-shadow.align 0 > $OUTDIR/dir0/fshadow.txt
# "-it 1" means perform 1 token iteration, since 0 is no longer allowed
$SANA_MULTI -it 1 -mode shadow -fshadow $OUTDIR/dir0/fshadow.txt > $OUTDIR/dir0/fshadow.stdout 2>$OUTDIR/dir0/fshadow.stderr
xz -f -5 "$SHADOW_GRAPH" & # compress in background to save space
...
if true; then
	...
	SHADOW_GRAPH="$OUTDIR/dir0/$NAME-shadow0.gw"
	wait; sleep 1; while [ -f "$SHADOW_GRAPH.xz" -a -f "$SHADOW_GRAPH" ]; do echo "waiting on 'xz $SHADOW_GRAPH'" >&2; sleep 1; done
	ls "$@" | awk '{file=$0;gsub(".*/",""); gsub(".el$",""); gsub(".gw$",""); printf "mkdir -p '$OUTDIR/dir-init';'"/usr/bin/time $TIME_VERBOSE timeout $TIME_LIMIT $SANA_MULTI $MEASURES $TYPEargs"' -multi-iteration-only -it 1 -fg1 %s -fg2 '$SHADOW_GRAPH.xz' -startalignment '$OUTDIR'/dir0/%s-shadow.align -o '$OUTDIR'/dir-init/%s >'$OUTDIR'/dir-init/%s.stdout 2>'$OUTDIR'/dir-init/%s.stderr\n", file,$0,$0,$0,$0}' | fgrep -v -f $OUTDIR/dir-init/schedule.done | tee $OUTDIR/dir-init/jobs.txt | eval timeout $TIME_LIMIT2 $PARALLEL
...
do
    ...
    i1=`expr $ITER + 1`
    if [ -f  $OUTDIR/dir$i1/$NAME-shadow$i1.gw.xz ]; then continue; fi
	...
	SHADOW_GRAPH="$OUTDIR/dir$ITER/$NAME-shadow$ITER.gw"
	wait; sleep 1; while [ -f "$SHADOW_GRAPH.xz" -a -f "$SHADOW_GRAPH" ]; do echo "waiting on 'xz $SHADOW_GRAPH'" >&2; sleep 1; done
	# compute iterations to perform based on the iterations-per-second computed in dir-init for this network
	SA_ITERS=`awk '/getPBad.*ips = /{ipsSum+=$NF}END{printf "%d\n", '$T_ITER'*60*ipsSum/NR}' $OUTDIR/dir-init/$bg.stdout`
	awkcel '{if(name!="'$bg'")next; ITER='$ITER';NUM_ITERS='$NUM_ITERS'; e0=log(tinitial);e1=log(tfinal);printf "'"mkdir -p $OUTDIR/dir$i1; /bin/rm -f networks/$NAME-shadow$ITER/autogenerated/$NAME-shadow${ITER}_UntypedNodes_Unlocked_Weighted.bin; /usr/bin/time $TIME_VERBOSE timeout $TIME_LIMIT $SANA_MULTI $MEASURES $TYPEargs -multi-iteration-only -it $SA_ITERS -fg1 $g -fg2 $SHADOW_GRAPH.xz -tinitial %g -tdecay %g -o $OUTDIR/dir$i1/$bg-shadow >$OUTDIR/dir$i1/$bg-shadow.stdout 2>$OUTDIR/dir$i1/$bg-shadow.stderr -startalignment $OUTDIR/dir$ITER/$bg-shadow.align"'", tinitial*exp((e1-e0)*ITER/NUM_ITERS),tdecay/NUM_ITERS;print ""}' $OUTDIR/dir-init/schedule.tsv
    ...
    SHADOW_GRAPHi1="$OUTDIR/dir$i1/$NAME-shadow$i1.gw"
    ShadowConfig "$SHADOW_GRAPHi1" $# "$@" 1 $OUTDIR/dir$i1/*-shadow.align 0 > $OUTDIR/dir$i1/fshadow.txt
    $SANA_MULTI -it 1 -mode shadow -fshadow $OUTDIR/dir$i1/fshadow.txt > $OUTDIR/dir$i1/fshadow.stdout 2> $OUTDIR/dir$i1/fshadow.stderr || die "$OUTDIR/dir$i1/$NAME-shadow$i1.gw network creation failed"
    xz -f -5 "$SHADOW_GRAPHi1" & # compress in background to save space
