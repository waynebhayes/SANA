...
void saveReport(const vector<Graph>& GV, Graph& SN, const MultiAlignment& A,
    ...
    if (reportFile == "") {
        reportFile = "alignments/" + graphNames + "/"
        + graphNames + "_" + method->getName();
  ...
  ofstream outfile;
  ofstream alignfile;
  outfile.open((reportFile + ".out").c_str(),ofstream::out);
  alignfile.open((reportFile + ".align").c_str(),ofstream::out);
  ...
  A.write(outfile);
  A.writeEdgeList(GV,SN, alignfile);
