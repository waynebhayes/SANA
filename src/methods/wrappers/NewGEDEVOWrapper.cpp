...
#include <iostream>
#include <sstream>
#include "NewGEDEVOWrapper.hpp"

using namespace std;

const string CONVERTER = "";
const string PROGRAM   = "gedevo";
const string OUTPUT_CONVERTER = "./NewGEDEVOoutput.sh";
...
NewGEDEVOWrapper::NewGEDEVOWrapper(Graph* G1, Graph* G2, string args): WrappedMethod(G1, G2, "GEDEVO", args) {
	wrappedDir = "wrappedAlgorithms/NewGEDEVO";
}

void NewGEDEVOWrapper::loadDefaultParameters() {
	parameters = "--maxsame 3000"; // maxsame 3000 is what they recommend, runtime many hours
}

string NewGEDEVOWrapper::convertAndSaveGraph(Graph* graph, string name) {
	graph->writeGraphEdgeListFormatPISWAP(name + ".txt" );
	return name + ".txt";
}

string NewGEDEVOWrapper::generateAlignment() {
...
Alignment NewGEDEVOWrapper::loadAlignment(Graph* G1, Graph* G2, string fileName) {
    return Alignment::loadPartialEdgeList(G1, G2, fileName, true);
}

void NewGEDEVOWrapper::deleteAuxFiles() {
