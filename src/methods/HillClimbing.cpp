...
Alignment HillClimbing::run() {
    ...
    bool needG2InducedEdges = (icsWeight > 0 or s3Weight > 0 or jsWeight > 0); // TODO: maybe remove the jsWeight, do i need it?
    int g2InducedEdges;
    ...
    if (jsWeight > 0) {
        ...
        vector<uint> jsAlignedByNode = js->getAlignedByNode(Alignment(A));
    }
