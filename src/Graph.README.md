# Graph Class Documentation for future developers (written by Nil Mamano) (amended and annotated by Marcus Longo)
...
*Marcus: This design decision was kept intact with my refactor. *
...
*Marcus: This was also kept intact, much to my own regret. The option still basically reconstructs a new graph, so please don't use it if you can help it.*
...
*Marcus: This was kept intact, however, the new structure does suggest that it might be worthwhile to consider a new constructor that can take in a vector of node references to produce a new graph.*
...
*Marcus: These getters are now obsolete. **Do not use them unless you know what you're doing.** Use deliverNode() instead.*
...
*Marcus: This is no longer true, as there is no formal adjacency matrix any longer. Instead, if an edge doesn't exist, it doesn't exist. There is no "edge" of weight 0 stored, although the graph will still return a 0 if you try to access a non-existent edge.*
