...
string GoSimilarity::getGoSimpleFileName(const Graph& G) {
	...
	if (fileExists(goFile)) {
		...
		error("GO data not available for this network");
		return "";
	}
}
...
void GoSimilarity::generateGene2GoSimpleFormat() {
	if (not fileExists("go/gene2go")) {
		error("Missing file \"go/gene2go\". It can be downloaded from \"ftp.ncbi.nlm.nih.gov/gene/DATA/gene2go.gz\"");
	}
	ifstream infile("go/gene2go");
...
vector<vector<uint> > GoSimilarity::loadGOTerms(const Graph& G) {
	string GName = G.getName();
	string GOFileInternalFormat = "networks/"+GName+"/autogenerated/"+GName+"_go_internal.txt";
...
void GoSimilarity::initSimMatrix() {
	...
	vector<vector<uint> > G1GOTerms = loadGOTerms(*G1);
	vector<vector<uint> > G2GOTerms = loadGOTerms(*G2);
