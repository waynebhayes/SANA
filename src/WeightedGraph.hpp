...
public:
    ...
    static WeightedGraph loadGraph(string name);
    static WeightedGraph multGraph(string name, uint path);
    ...
    WeightedGraph(uint n, const vector<vector<ushort> > edgeList);
    ...
    // Remove functions until later time
    ...
    WeightedGraph randomNodeInducedSubgraph(uint numNodes);

    bool isWellDefined();

    bool sameNodeNames(const WeightedGraph& other) const;
    ...
    void reIndexWeightedGraph(map<ushort, ushort> reIndexMap);  // Changes the node indexes according to the map
