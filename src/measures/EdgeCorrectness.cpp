...
int EdgeCorrectness::getIncChangeOp(uint peg, uint oldHole, uint newHole, const Alignment& A) {
    return computeIncChangeOp(peg, oldHole, newHole, A);
}

int EdgeCorrectness::computeIncChangeOp(uint peg, uint oldHole, uint newHole, const Alignment& A) {
    int res = 0;
...
int EdgeCorrectness::getIncSwapOp(uint peg1, uint peg2, uint hole1, uint hole2, const Alignment& A) {
    return computeIncSwapOp(peg1, peg2, hole1, hole2, A);
}

int EdgeCorrectness::computeIncSwapOp(uint peg1, uint peg2, uint hole1, uint hole2, const Alignment& A) {
#ifdef WEIGHT
    throw runtime_error("EdgeCorrectness::computeIncSwapOp should not be called with WEIGHT");
    return 0;
#else
    int res = 0;
