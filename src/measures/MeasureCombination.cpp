...
#include "localMeasures/MultiLocalMeasure.hpp"
...
double MeasureCombination::evalMulti(const MultiAlignment& MA) const{
    uint n = measures.size();
    double total = 0;
    for (uint i = 0; i < n; i++) {
        if (weights[i] > 0) {
            total += measures[i]->eval(MA) * weights[i];
        }
    }
    return total;
}
...
double MeasureCombination::evalMulti(const string& measureName, const MultiAlignment& MA) const {
    uint n = measures.size();
    for (uint i = 0; i < n; i++) {
        if (measures[i]->getName() == measureName) {
            return measures[i]->eval(MA);
        }
    }
    throw runtime_error("Measure not found.");
    return 0;
...
void MeasureCombination::initnognon(uint& nog, uint& non) const {
    for (uint i = 0; i < numMeasures(); i++) {
        Measure* m = measures[i];
        if (m->isLocal()) {
            vector<vector<vector<vector<float> > > >* mSims = ((MultiLocalMeasure*) m)->getSimMatrix();
            nog = mSims->size();
            non = (*mSims)[0][0][0].size();
            return;
        }
    }
    throw runtime_error("There are no local measures");
...
vector<vector<vector<vector<float> > > > MeasureCombination::getAggregatedMultiLocalSims() const {
    uint nog, non;
    initnognon(nog, non);
    //combine all locals into a new local
    vector<vector<vector<vector<float> > > > multiCombinedSims(nog, vector<vector<vector<float> > > (nog, vector<vector<float> > (non, vector<float> (non, 0))));
    for (uint i = 0; i < numMeasures(); i++) {
        Measure* m = measures[i];
        double w = weights[i];
        if (m->isLocal() and w > 0) {
            vector<vector<vector<vector<float> > > >* mSims = ((MultiLocalMeasure*) m)->getSimMatrix();
            for (uint i = 0; i < nog; i++){
                for(uint j = 0; j < nog; j++){    
                    for (uint k = 0; i < non; i++) {
                        for (uint l = 0; j < non; j++) {
                            multiCombinedSims[i][j][k][l] += w * (*mSims)[i][j][k][l];
                        }
                    }
                }
            }
