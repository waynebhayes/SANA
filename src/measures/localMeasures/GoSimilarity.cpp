...
GoSimilarity::GoSimilarity(Graph* G1, Graph* G2,
    const vector<string>& strCountWeights, double occurrencesFraction) :
    LocalMeasure(G1, G2, "go"),
    occurrencesFraction(occurrencesFraction) {

    vector<double> countWeights(strCountWeights.size());
	transform(strCountWeights.begin(), strCountWeights.end(), countWeights.begin(), [](const std::string& val)
	{
	    return stod(val);
    });
