...
SANA* initSANA(vector<Graph>& GV,Graph& SN, ArgumentParser& args, MeasureCombination& M){
    double TInitial = 0;
    ...
    SANA* sana;
    double minutes = args.doubles["-t"];
    string scoreName = "msec";
    cerr<<"check at::run before sana = new SANA(&GV,... @  methodSelector.cpp "<<endl;
    sana = new SANA(&GV, &SN, TInitial, TDecay, minutes, &M, scoreName);
    cerr<<"check at::run after sana = new SANA(&GV,... @  methodSelector.cpp "<<endl;
    /*if(args.doubles["-i"] > 0){
...
Method* initMethod(Graph& G1, Graph& G2, ArgumentParser& args, MeasureCombination& M) {
    ...
    /*std::cout<<"\n starting printing initmethod\n";
	std::cout<<"name :"<<name<<"\n";
	std::cout<<"startAligName :"<<startAligName<<"\n";
	std::cout<<"alpha :"<<alpha<<"\n";
	std::cout<<"wrappedArgs :"<<wrappedArgs<<"\n\n\n";*/
    if (name == "greedylccs")
