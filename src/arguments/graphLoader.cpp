...
//only for multi-alignment

void initGraphs(vector<Graph>& graphs, ArgumentParser& args)
{
	createFolder("networks");
	vector<string> graphNames = vector<string>(args.vectors["-multialign"]);
    vector<string> trimmedGraphNames = vector<string>(0);
	string graphGWFile;
	for(uint i = 0;i <graphNames.size();i++)
	{
		//graphFolders.push_back("networks/"+g1Name);
		createFolder("networks/"+graphNames[i]);
		createFolder("networks/"+graphNames[i]+"/autogenerated");
		graphGWFile = "networks/"+graphNames[i]+"/"+graphNames[i]+".gw";

		if (not fileExists(graphGWFile)) {
			throw runtime_error("File not found: "+graphGWFile);
		}
		if(!(find(trimmedGraphNames.begin(), trimmedGraphNames.end(), graphNames[i]) != trimmedGraphNames.end())){
			trimmedGraphNames.push_back(graphNames[i]);
		}
	}
	...
	T.start();
	Graph temp;
	cout<<"check@graphLoader.cpp in second initGraphs at the end \n";
	for(uint i = 0;i <trimmedGraphNames.size();i++){
		temp = Graph::loadGraph(trimmedGraphNames[i]);
		cout<<"temp.getName() is:"<<temp.getName()<<endl;
    	graphs.push_back(temp);
	}
	for(uint i = 0;i <graphs.size();i++){
        cout<<"graphs["<<i<<"].getName() is:"<<graphs[i].getName()<<endl;
	}
	for(uint i = 0;i <graphs.size();i++){
        cout<<"graphs["<<i<<"].getNumNodes() is:"<<graphs[i].getNumNodes()<<endl;
	}
	/*cout<<"graphs[0].getName() is:"<<graphs[0].getName()<<endl;
	cout<<"graphs[1].getName() is:"<<graphs[1].getName()<<endl;
	cout<<"graphs[2].getName() is:"<<graphs[2].getName()<<endl;
	cout<<"graphs[0].getNumNodes() is:"<<graphs[0].getNumNodes()<<endl;
	cout<<"graphs[1].getNumNodes() is:"<<graphs[1].getNumNodes()<<endl;
	cout<<"graphs[2].getNumNodes() is:"<<graphs[2].getNumNodes()<<endl;*/
}
