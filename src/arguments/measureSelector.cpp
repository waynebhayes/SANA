...
#include "../measures/localMeasures/MultiLocalMeasure.hpp"
#include "../measures/localMeasures/MultiSequence.hpp"
#include "../measures/localMeasures/MultiImportance.hpp"
...
double totalMultiGenericWeight(ArgumentParser& args) {
    vector<string> optimizableMultiMeasures = {
        "msec", "ciq", "multiimportance", "multisequence"
    };
    double total = 0;
    for (uint i = 0; i < optimizableMultiMeasures.size(); i++) {
        total += args.doubles["-"+optimizableMultiMeasures[i]];
    }
    return total;
}

double getMultiWeight(string measureName, vector<Graph>& GV, Graph& SN, ArgumentParser& args){
    string objFunType = args.strings["-objfuntype"];
    if (objFunType == "generic") {
        double weight = args.doubles["-"+measureName];
        return weight/totalMultiGenericWeight(args);
    }
    else
    {return 0;}
}
...
void initMeasures(MeasureCombination& M, vector<Graph>& GV, Graph& SN, ArgumentParser& args){
    ...
    Measure *m;
    m = new MultiSymmetricEdgeCoverage(&GV, &SN,"msec");
    M.addMeasure(m,1);
}
bool shouldInitMulti(string measureName, vector<Graph>& GV, Graph& SN, ArgumentParser& args) {
    double weight = getMultiWeight(measureName, GV, SN, args);
    if (weight > 0) return true;
    return false;
}
