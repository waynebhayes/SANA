...
void initMeasures(MeasureCombination& M, const Graph& G1, const Graph& G2, ArgumentParser& args) {
    ...
    Measure *m;
    m = new EdgeCorrectness(&G1, &G2); M.addMeasure(m, getWeight("ec", G1, G2, args)); 
    M.addMeasure(m, getWeight("ec1", G1, G2, args)); 
    // m = new EdgeCorrectness(&G2, &G1); M.addMeasure(m, getWeight("ec2", G1, G2, args)); // ec2 has been depracated
