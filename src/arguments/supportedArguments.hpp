...
using namespace std; //To make my life easier so I didn't have to type std:: before many expressions in my program.
...
void validateAndAddArguments(); //Checks to make sure fields inside supportedArguments are properly filled out and adds them to the four __Args datatypes.
...
string formatWithNewLines(const string &item4); //helper function of formatDescription() that outputs a a description of an argument on multiple lines when hasNewLines() evaluates to true.
