...
std::vector<PointComparison> nearest_neighbor_kdtree(const std::vector<Point>& query, const std::vector<Point>& target, bool do_cosine) {
    ...
    std::vector<PointComparison> comp_v;
    comp_v.reserve(query_mp.size());
    ...
    for (const auto& qmp : query_mp) {
        ...
        PointComparison c{ qi.id, ti.id, std::sqrt(out_dist_sqr), angle_diff };
        comp_v.push_back(c);
    }
    return comp_v;
}

std::vector<PointComparison> nearest_neighbor(const std::vector<Point>& query, const std::vector<Point>& target, bool do_cosine) {
    std::vector<PointComparison> comp_v;
    comp_v.reserve(query.size());
    for (const auto& query_i : query) {
        ...
        PointComparison c{ query_i.id, target_min.id, distance_min, angle_diff };
        comp_v.push_back(c);
    }
    return comp_v;
}
    ...
    switch (md) {
        ...
        case mode::listed: {
            ...
            for(int i = optind; i < argc; i++) {
                ...
                std::cerr << "processing " << query_filepath << " " << target_filepath << "\n";
        ...
        case mode::read_matrix: {
            ...
            std::vector<std::vector<int>> matrix(MATRIX_DISTANCE_BIN_COUNT, 
                std::vector<int>(MATRIX_THETA_BIN_COUNT, 0));
            rc = read_matrix(matrix_filepath, matrix);
            assert(rc == 0);
            print_matrix(matrix);
            return 0;
        }
