...
\ElsIf {$P(\Delta E, T(i))>\mbox{random}(0,1)$} $a\gets a'$
\EndIf
\State $i \gets i+1$
\EndWhile
\State \Return $a$
...
%Despite the capacity of simulated annealing to escape local maximums, the final result is still greatly conditioned by the initial stages of the process. Some initial starting points may be too far from any good solution, in the sense that it would take far too many individual changes or swaps (see Figure~\ref{fig:operators}) to get from the initial point to a good solution.
%
%As an example, over 100 runs of SAGA optimizing S3 with the networks from the Yeast and Human dataset (see section~\ref{datadescription}) with a maximum execution time of 12 minutes, the average S3 score was 0.38564, while the minimum and maximum were 0.357954 and 0.409462 respectively.  Therefore, we implement a restart scheme such that when an alignment seems unpromising, it is discarded early on and we start with a new random initial state.
%
%% explain the restart scheme
%We divide the execution of SAGA into three phases.
...
%
%\begin{algorithm}
%\caption{SAGA with restarts}\label{alg:restart}
%\begin{algorithmic}[1]
...
%
%Following with the previous example, over 30 runs of SAGA  with restarts optimizing S3 with the networks from the Yeast and Human dataset and setting $N=3\cdot 10^7, K=15, t_1=20$ minutes, $t_2=2$ minutes, $t_3=10$ minutes for a total execution time of one hour, the average S3 score was 0.39573, while the minimum and maximum were 0.384189 and 0.407284 respectively. Note that with this set of values for $K,t_1,t_2,$ and $t_3$, the total execution time of the final alignment is roughly 12 minutes.
...
% A similar comparison optimizing HubAlign's topological importance-based measure is not possible because in HubAlign the objective function and the search algorithm complement each other: edge coverage is only enforced through the search algorithm. Thus, SAGA does not obtain good alignments when optimizing only its objective function.
