...
ssize_t** kuhn_match(cell** table, size_t n, size_t m)
{
    ...
    size_t* altRow = new size_t[n*m];
    size_t* altCol = new size_t[n*m];
...
BitSet new_BitSet(size_t size)
{
    ...
    this_.limbs = new llong[c];
    this_.prev = new size_t[c+1];
    this_.next = new size_t[c+1];
    *(this_.first = new size_t) = 0;
    
    size_t i;
    for (i = 0; i < c; i++)
    {
        *(this_.limbs + i) = 0LL;
        *(this_.prev + i) = *(this_.next + i) = 0L;
    }
    *(this_.prev + c) = *(this_.next + c) = 0L;
 ...
 * 
 * @param  this_  The bit set
 * @param  i     The index of the bit to turn on
 */
void BitSet_set(BitSet this_, size_t i)
{
    size_t j = i >> 6L;
    llong old = *(this_.limbs + j);
    
    *(this_.limbs + j) |= 1LL << (llong)(i & 63L);
 ...
 */
ssize_t BitSet_any(BitSet this_)
{
    if (*(this_.first) == 0L)
        return -1;
    
    size_t i = *(this_.first) - 1;
    return (ssize_t)(lb(*(this_.limbs + i) & -*(this_.limbs + i)) + (i << 6L));
}
