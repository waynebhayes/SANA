/*
 * scoremodelcoexpressioncontinuous.h
 *
 *  Created on: 17-jan-2012
 *     Authors: M. El-Kebir and M.E. van der Wees
 */

#ifndef SCOREMODELCOEXPRESSIONCONTINUOUS_H_
#define SCOREMODELCOEXPRESSIONCONTINUOUS_H_
...
class ScoreModelCoexpressionContinuous : public ScoreModel<GR, BGR>
{
public:
...
public:
  /// Constructor
  ScoreModelCoexpressionContinuous(const MatchingGraph<Graph, BpGraph>& matchingGraph, double beta, double corrThreshold);
  /// Destructor
  virtual ~ScoreModelCoexpressionContinuous() {}

  std::string getScoreFunction() const { return "Continuous 1"; }
  ...
  double getWeightG1G2(Edge ij, Edge kl) const
  {
    ...
    // Profit for equal signs in coexpression values
      ...
      return _beta * ((1 - _corrThreshold) - fabs(weight1 - weight2)) / (1 - _corrThreshold);
  }
};

template<typename GR, typename BGR>
inline ScoreModelCoexpressionContinuous<GR, BGR>::ScoreModelCoexpressionContinuous(const MatchingGraph<Graph, BpGraph>& matchingGraph, double beta, double corrThreshold)
...
#endif /* SCOREMODELCOEXPRESSIONCONTINUOUS_H_ */
