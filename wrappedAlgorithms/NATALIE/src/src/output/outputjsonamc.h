...
#ifndef OUTPUTJSONAMC_H
#define OUTPUTJSONAMC_H
...
class OutputJsonAmc : public Output<GR, BGR>
{
public:
...
public:
  OutputJsonAmc(const MatchingGraphType& matchingGraph,
                int maxNodes,
...
template<typename GR, typename BGR>
inline void OutputJsonAmc<GR, BGR>::write(const BpMatchingMapType& matchingMap,
                                       ...
                                       std::ostream& out) const
{
  ...
  for (EntryVectorIt it1 = entries.begin(); it1 != entries.end(); it1++, nodeIdx1++)
  {
    ...
    for (EntryVectorIt it2 = it1; it2 != entries.end(); it2++, nodeIdx2++)
    {
      ...
      {
        Edge edgeG1 = _matchingGraph.getEdgeG1(it1->_node1, it2->_node1);
        Edge edgeG2 = _matchingGraph.getEdgeG2(it1->_node2, it2->_node2);

        out << "\t\t{ \"source\":" << nodeIdx1
            << ",\"target\":" << nodeIdx2 << "," << "\"cat\":2"
            << ",\"conf1\":\"" << _matchingGraph.getWeightG1(edgeG1) << "\""
            << ",\"conf2\":\"" << _matchingGraph.getWeightG2(edgeG2) << "\""
            << ",\"coexp\":\"" << std::setprecision(3) << _scoreModel.getWeightG1G2(edgeG1, edgeG2) << "\""
            << " }";
      }
      else if (queryEdge)
      {
