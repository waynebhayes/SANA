...
template<class edge, class cap_array>
class edge_smaller {
 const cap_array&  cap;
public:
...
edge_filter(const graph_t& g, const cap_array& _cap, const flow_array& _flow, 
                              const cost_array& _cost, const pot_array& _pot, 
                              double af, int eps) : 
                              G(g),cap(_cap),flow(_flow),cost(_cost),pot(_pot)
{ af_bound = af*eps; }
...
template <class graph, class succ_pred_array>
class node_level_queue16 {
...
node_level_queue16(const graph& G, succ_pred_array& sp, int n, node s)
            : succ_pred(sp), senti(s)
{ T = new node[n];
  for(int i=0; i<n; i++) T[i] = s;
  first_v = G.first_node();
  max = -1;
  unsigned short* p = (unsigned short*)&succ_pred[s];
  p[0] = p[1] = s-first_v;
 }

~node_level_queue16() { delete[] T; }

node get_next(node v) const
{ unsigned short* p = (unsigned short*)&succ_pred[v];
  return first_v + p[0]; 
 }

node get_pred(node v) const
{ unsigned short* p = (unsigned short*)&succ_pred[v];
  return first_v + p[1]; 
 }

void set_next(node v, node x)
{ unsigned short* p = (unsigned short*)&succ_pred[v];
  p[0] = x-first_v;
 }

void set_pred(node v, node x) 
{ unsigned short* p = (unsigned short*)&succ_pred[v];
  p[1] = x-first_v;
 }
...
void pop(node v, int i) { T[i] = get_next(v); }
...
class zero_node_array {
typedef typename graph_t::node node;
public:
zero_node_array(const graph_t&, int) {}
int operator[](node) const { return 0; }
bool use_node_data(const graph_t&, int) { return true; }
};

template <class graph_t>
class zero_edge_array {
typedef typename graph_t::edge edge;
public:
zero_edge_array(const graph_t&, int) {}
int operator[](edge) const { return 0; }
bool use_edge_data(const graph_t&, int) { return true; }
};
...
template<class cap_array, class flow_array, class cost_array>
bool topsort(const graph_t& G, //const excess_array& excess,
                               const cap_array& cap,
                               const flow_array& flow,
                               const cost_array& cost,
                               const pot_array& pot,
    ...
    { 
      ...
      if (i == 0)
        { e = G.first_out_edge(v);
         ...
         if (i < 0)
            { e = G.next_in_edge(e_first-i);
              if (e) i = -(e-e_first);
              else i = 0;
             }
         else
            { e = G.next_out_edge(e_first+i,v);
              if (e) 
                 i = e-e_first;
              else
                 { e = G.first_in_edge(v);
                                           ...
                                           dist_array& dist) 
{
  ...
  if (!topsort(G,cap,flow,cost,pot,Q))
  { delete[] Q;
    return false; 
   }
...
if (write_log)
  cout << string("initial flow: %d empty  %d full",count2,count1) << endl;
...
#if defined(NODE_SIZE_LOG)
  node_level_queue16<graph_t,succ_array> PQ(G,succ,inf,G.stop_node());
#else
/*
   ...
   forall_nodes(v,G)
    ...
    { cout << string("|active| = %d",active.size()) << endl;
      cout << string("eps = %9d cost = %12.0f pot = %14.0f fixed = %5d",
                      eps, total_cost(G,cost,flow,n+1), total_pot(G,pot),
                      af_count) << endl;
     }
...
*/
    ...
    { int count1 = 0;
      ...
      { edge e;
        forall_out_edges(e,v)
        { if (out_cap(e) == 0) count1++;
          if (in_cap(e) == 0) count2++;
         }
       }
      cout << string("after refine: %d empty  %d full",count2,count1) << endl;
      cout << endl;
    }
...
/*
  ...
  float tt = used_time();
  G.sort_edges(edge_smaller<edge,cap_array>(cap));
                           ...
                           int b = 16)
{
  ...
  excess_array excess(G,0);      // supply values after elimination of
...
/*
  float tt = used_time();
  G.sort_edges(edge_smaller<edge,ucap_array>(cap));
  if (write_log) cout << string("sorting: %.2f sec",used_time(tt)) << endl;
*/
