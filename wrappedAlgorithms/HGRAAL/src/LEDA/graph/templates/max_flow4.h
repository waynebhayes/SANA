...
{ 
  ...
  node_array<node,graph> min_node(G,0);

  node_array<int,graph> min_dist(G,n);
  ...
  while ((v = U.del_max()) != t) 
  {
    ...
    for(;;)
    {
      ...
      { insp_count++;
        ...
        else if (dist[w] < dmin) { dmin = dist[w]; vmin = w; cur_i = (e-f_edge); }
       }
