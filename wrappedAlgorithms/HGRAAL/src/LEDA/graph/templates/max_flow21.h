...
{ 
  ...
  bool update_in_edges = (m/50 > n);

  g_first_in_array<graph> g_first_in;
  g_next_in_array<graph> g_next_in;

 compute_dist1(G,V,s,t,cap,flow,excess,dist,U,count,g_first_in,g_next_in,false);

  node_array<edge,graph> first_in;
  edge_array<edge,graph> next_in;
    ...
    for(;;)
    {
      ...
      { insp_count++;
        ...
        { push_count++;
          NT ew = excess[w];
          if (ew == 0) U.insert_non_max(w,dw);
          if (update_in_edges && f == 0) G.push_in_edge(w,e); 
          if (ev <= x) 
          { flow[e] += ev; 
            excess[w] += ev;
            excess[v] = 0; 
      ...
      { insp_count++;
        ...
        { push_count++;
          ...
          { flow[e] -= ev; 
            excess[w] += ev;
            excess[v] = 0; 
            if (update_in_edges) v->FirstInEdge = e_stop->NextInEdge;
            goto NEXT_NODE;
           }
          flow[e] -= x;
          excess[w] += x;
      ...
      { push_count++;
        ...
        if (emin_last == 0) // out-edge
          { if (update_in_edges && flow[emin] == 0) G.push_in_edge(w,emin); 
            flow[emin] += ev;
           }
        else
          { if (update_in_edges && ev == rmin) 
            { if (emin_last == e_stop)
