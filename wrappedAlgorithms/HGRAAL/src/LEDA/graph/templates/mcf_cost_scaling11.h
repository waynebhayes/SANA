...
ff_edge_filter(const graph_t& g, const cost_array& c, const pot_array& p, int e)
   : G(g), cost(c), pot(p), eps(e) { }


bool operator()(edge e, node s, node t)
{ rcost_t rc = rcost(e,s,t);
  return (rc <= eps && rc >= -eps);
}
...
//int eps_opt = (1 << 16)/n;
  ...
  while (eps >= eps_opt)
  {
    ...
    { edge_filter<graph_t,cap_array,flow_array,cost_array,pot_array,rcost_t> 
         F(G,cap,flow,cost,pot,af_factor*alpha);
...
*/
   ...
   { feasible_flow<int,graph_t,succ_array,dist_array> ff;
     ff_edge_filter<graph_t,cost_array,pot_array,rcost_t> filt(G,cost,pot,eps);
...
node v;
forall_nodes(v,G) 
{ if (excess[v] > 0) total_excess += excess[v];
  else  total_demand -= excess[v];
} 
cout << "excess = " << total_excess << "   demand = " << total_demand << endl;
*/

        ff.feasible(G,excess,cap,flow,filt);
