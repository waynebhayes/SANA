...
// $Revision: 1.6 $  $Date: 2005/04/14 10:45:15 $
...
max_flow() : node_slots(3), edge_slots(0), h(5.0),fval(0),cut_bound(0)
{ reset_counters(); }
...
void compute_dist00(const graph& G, node* V, node s, node t,
                   excess_array& excess,
                   dist_array& dist,
                   level_queue& U,
                   int* count)
...
template<class cap_array, class flow_array>
void compute_dist1(const graph& G, node* V,node s, node t,
                   const cap_array& cap,
                   const flow_array& flow,
                   excess_array& excess,
                   dist_array& dist,
...
*/
  ...
  phase0(G,V,s,t,cap,flow,excess,dist,count,U);
...
{ 
  out << endl;
  out << "          pushes     relabeles  inspects   updates gaps g-nodes time";
  out << endl;
  out << "--------------------------------------------------------------------";
  out << endl;
