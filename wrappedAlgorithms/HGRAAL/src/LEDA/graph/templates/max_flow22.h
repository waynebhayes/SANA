...
// $Revision: 1.4 $  $Date: 2005/04/14 10:45:15 $
...
class g_first_in_array {
 typedef typename graph::node node;
 typedef typename graph::edge edge;
public:
 edge operator[](node v) const { return (edge)v->FirstInEdge; }
};


template <class graph>
class g_next_in_array {
 typedef typename graph::edge edge;
public:
 edge operator[](edge e) const { return (edge)e->NextInEdge; }
};
...
bool check_in_edges(const graph& G, const flow_array& flow)
{ 
  ...
  forall_nodes(v,G)
  { edge e;
    forall_out_edges(e,v) out_flow += flow[e];
    forall_in_edges(e,v) 
    { if (flow[e] == 0) 
      { count++;
        cout << "flow == 0: e = " << G.index(e) <<endl;
       }
      in_flow += flow[e];
     }
   }

  if (count > 0) cout << "check_in_edges: zero r-capacity edges" << endl;
  if (out_flow != in_flow) cout << "missing non-zero edges" << endl;

  return (count == 0 && out_flow == in_flow);
}


template<class flow_array, class first_in_array, class next_in_array>
int build_in_edges(const graph& G, const flow_array& flow,
                                    first_in_array& first_in,
                                    next_in_array& next_in)
{ 
  node v;
  forall_nodes(v,G)
  { first_in[v] = (edge)v->FirstInEdge;
    v->FirstInEdge = 0;
   }

  edge e;
  forall_edges(e,G) next_in[e] = (edge)e->NextInEdge;
  ...
  { edge e;
    forall_out_edges(e,v) 
    { node w = target(G,e,v);
      if (flow[e] > 0) { G.push_in_edge(w,e); count++; }
     }
   }
...
void qsort_outdeg(const graph& G, node* l, node* r) const
{ if (r <= l) return;
  if (G.outdeg(*r) < G.outdeg(*l)) swap(*l,*r);
  if (r == l+1) return;
  node* k = l + (r-l)/2;
  if (G.outdeg(*k) < G.outdeg(*l))
     swap(*l,*k);
  else
     if (G.outdeg(*r),G.outdeg(*k)) swap(*k,*r);
  if (r == l+2) return;
  node* i = l+1;
  node* j = r;
  swap(*i,*k);
  k = i;
  for(;;)
  { while (G.outdeg(*++i) < G.outdeg(*k));
    while (G.outdeg(*k)   < G.outdeg(*--j));
    if (i<j) swap(*i,*j);
    else break;
   }
  swap(*k,*j);
  if (j > l+1) qsort_outdeg(G,l,j-1);
  if (r > j+1) qsort_outdeg(G,j+1,r);
}



/*
void topsort(const graph& G, excess_array& excess, dist_array& dist,
                                                   level_queue& U)
{
  int n = G.number_of_nodes();

  node_array<int,graph> indeg(G,0);
  b_queue<node> zero(n);
  ...
  forall_nodes(v,G)
  { edge e;
    forall_out_edges(e,v) indeg[target(G,e,v)]++;
   }
  ...
  while (!zero.empty())
   { count++;
     node v = zero.pop();
     if (dist[v] < n && excess[v] > 0) U.insert(v,dist[v]);
     edge e;
     forall_out_edges(e,v)
     { node w = target(G,e,v);
...
{ 
  ...
  { 
    ...
    { edge e;
      ...
      forall_out_edges(e,v)
      { node u = target(G,e,v); 
        if (u != v && dist[u] <= d) cut -= (cap[e] - flow[e]);
       }
...
template<class cap_array, class flow_array, class first_in_array, 
                                                  class next_in_array>
...
{ 
  ...
  { 
    ...
    { edge e;
      ...
      for(e=first_in[v]; e; e = next_in[e])
      { if (flow[e] == cap[e]) continue;
        node u = source(G,e,v); 
        if (dist[u] == n)
        { dist[u] = d+1;
...
{ 
  ...
  while ((v = U.del_max()) != t) 
  {
    for(;;)
    {
      ...
      if (ev <= (cap[emin] - flow[emin]))
      { push_count++;
        node w = target(G,emin,v);
...
template<class cap_array, class flow_array>
void phase1_0(const graph& G, node* V, node s, node t, cap_array& cap,
                                                     flow_array& flow,
                                                     excess_array& excess,
                                                     dist_array& dist,
                                                     int* count,
...
{ 
  ...
  g_first_in_array<graph> g_first_in;
  g_next_in_array<graph> g_next_in;
  compute_dist1(G,V,s,t,cap,flow,excess,dist,U,count,g_first_in,g_next_in,0);
  ...
  while ((v = U.del_max()) != t) 
  {
    for(;;)
    {
      ...
      { V_sz = compute_dist1(G,V,s,t,cap,flow,excess,dist,U,count,g_first_in,
                                                                  g_next_in);
...
template<class cap_array, class flow_array>
void phase1_1(const graph& G, node* V, node s, node t, cap_array& cap,
                                                     flow_array& flow,
                                                     excess_array& excess,
                                                     dist_array& dist,
                                                     int* count,
...
{ 
  ...
  node_array<edge,graph> first_in(G);
  edge_array<edge,graph> next_in(G);
  build_in_edges(G,flow,first_in,next_in);


  edge e_stop = G.stop_edge();
  ...
  while ((v = U.del_max()) != t) 
  {
    ...
    for(;;)
    {
      ...
      edge e_last = e_stop;
      e_last->NextInEdge = v->FirstInEdge;
      ...
      { insp_count++;
        ...
        { push_count++;
          ...
          if (ev >= x) 
            e_last->NextInEdge = e->NextInEdge;
          else 
            e_last = e;
          ...
          { flow[e] -= ev; 
            excess[w] += ev;
            excess[v] = 0; 
            v->FirstInEdge = e_stop->NextInEdge;
            goto NEXT_NODE;
           }
          flow[e] -= x;
          excess[w] += x;
          ...
          { if (v != w && dw < dmin) 
            { dmin = dw; emin = e; emin_last = e_last; rmin = x; }
            e_last = e;
           }
      }
      ...
      { push_count++;
        ...
        if (excess[w] == 0) U.insert_non_max(w,dmin);
        excess[w]  += ev;
        if (emin_last == 0) // out-edge
          { if (flow[emin] == 0) G.push_in_edge(w,emin); 
            flow[emin] += ev;
           }
        else
          { if (ev == rmin) 
            { if (emin_last == e_stop)
                v->FirstInEdge = emin->NextInEdge;
              else
                emin_last->NextInEdge = emin->NextInEdge;
             }
            flow[emin] -= ev;
           }
        excess[v]  = 0;
      ...
      if ((h > 0 && insp_count  >= limit_heur) || 
          (h < 0 && relabel_count >= limit_heur))
      { V_sz = compute_dist1(G,V,s,t,cap,flow,excess,dist,U,count,first_in,
                                                                  next_in);
        limit_heur += heuristic;
        goto NEXT_NODE;
       }
...
*/
  ...
  if (m < 50*n)
    phase1_0(G,V,s,t,cap,flow,excess,dist,count,U);
  else
    phase1_1(G,V,s,t,cap,flow,excess,dist,count,U);
